...
- OpenClaw v2026.7.1 or later (latest stable; tested with v2026.7.1-2) (`npm install -g openclaw@latest`)
...
> **Note:** The `models.providers.ollama.retry` config key is NOT supported by OpenClaw's zod schema (as of v2026.7.1-2). The only way to clear cooldown is to edit `auth-profiles.json` directly and restart the gateway.
