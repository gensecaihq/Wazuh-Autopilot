...
Wazuh Autopilot is now a Strands Agents platform. This is a breaking change: the Node Runtime Service and its installer are gone.

### Changed
- **Strands Agents core**: the platform previously under `strands/` is now the project root (`backend/`, `ui/`, `mock-wazuh/`, `Dockerfile`, `docker-compose.yml`). It's a 13-agent Strands swarm (Swarm/Graph workflows), a web console, RBAC, an autonomy policy, an approvals queue, MCP execution with `wazuh_check_*` verification and rollback, run tracing, evals, and Bedrock/Anthropic/OpenAI-compatible/vLLM/LiteLLM/Ollama providers. It talks to the Wazuh MCP Server directly.
- **Strands is the only agent engine.** OpenClaw, Hermes and NemoClaw are no longer supported. An audit found that OpenClaw agents only reached Wazuh through two proxied endpoints (versus 41 MCP tools for the Strands agents), carried their own diverging 2,200-line prompt set instead of the verified skills, had two dormant agents and timer-driven heartbeats outside the policy engine, and passed tokens in URLs. The Hermes profile either got no Wazuh tools or unfiltered action tools. NemoClaw inherited the OpenClaw issues. Their profiles, the Runtime API compatibility layer and the setup-wizard runtime step were removed. Nemotron models remain available through the new **NVIDIA NIM** provider preset.
- **IR playbooks are skills**: the 7 playbooks (brute force, lateral movement, privilege escalation, ransomware, suspicious PowerShell, data exfiltration, vulnerability spike) are now `ir-playbook-*` Agent Skills with the full playbook as `references/playbook.md`, carried by the IR lead, DFIR investigator and vulnerability manager (37 skills in total after the audit).
- Ollama models default to a 32k context window (configurable).
- CI runs pytest, the UI build, a mock-server smoke test and Docker builds with a Trivy scan.

### Audit fixes (Wazuh accuracy and tool consistency)
- **Rule IDs verified against the official Wazuh 4.14.8 ruleset.** The IR playbooks cited many rule IDs that exist but mean something else (e.g. rootcheck/CIS-CAT/Chrome-Remote-Desktop rules presented as Windows logon, exfiltration or WinRM detections), plus nonexistent ones; vulnerability-detector severities were inverted. All 37 skills now cite verified IDs and levels; behaviors without stock rules are marked as `custom rule 1000xx` sketches. `test_wazuh_skills.py` enforces this from `tests/fixtures/wazuh_rule_refs.json` (regenerated by `scripts/build_wazuh_rule_refs.py`).
- **9 new Wazuh skills**: wazuh-mcp-querying (every agent), wazuh-rules-and-decoders, wazuh-fim-investigation, wazuh-sca-hardening, wazuh-malware-detection, wazuh-windows-sysmon, wazuh-cloud-container, wazuh-active-response, wazuh-platform-health. The 21 core skills were corrected (tool parameters, what `wazuh_check_*` actually inspects, permanent API blocks, compliance tags, ATT&CK mappings from the ruleset).
...
### Removed
- `openclaw/`, `hermes/`, `nemoclaw/` agent profiles and their docs (HERMES_DEPLOYMENT, NEMOCLAW_DEPLOYMENT).
- `runtime/autopilot-service` (Node Runtime Service, its Slack Socket Mode integration and its 594-test suite), `install/`, `scripts/`, `policies/` (replaced by the platform's policy engine and action catalog), legacy `agents/*.agent.yaml`, root-level `playbooks/` (now skills), and the docs specific to the Node runtime (Runtime API, evidence pack schema, CLI reference, Slack Socket Mode, heartbeats, observability export, policy and approvals, agent configuration/communication, scenarios, vLLM and air-gapped guides; the last two are folded into `docs/MODEL_PROVIDERS.md`).

### Docs
- New: QUICKSTART, DEPLOYMENT, ARCHITECTURE, AGENTS_AND_SKILLS, AUTONOMY_AND_APPROVALS, MODEL_PROVIDERS, MCP_INTEGRATION, TROUBLESHOOTING, API. TAILSCALE_MANDATORY, SECURITY and CONTRIBUTING are rewritten for the new architecture. Tests: 41 backend tests.

## [1.x] - Node runtime changes (never tagged; superseded by 3.0.0)
