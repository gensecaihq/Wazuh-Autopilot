...
The stock Wazuh ruleset has little exfiltration coverage; the rules in this playbook numbered 100501 and up are
**org-custom** and must be created (range 100000–120000). Stock rules that do help are named explicitly.

| Technique | Wazuh Detection Method | Rules |
|-----------|------------------------|----------|
| T1041 | Network flow analysis + C2 IOC correlation | custom rules 100501, 100502 (firewall/NetFlow volume) |
| T1048.001 / T1048.003 | HTTPS volume anomalies, DNS tunnelling | custom rules 100520, 100521, 100522 (DNS server or Zeek logs) |
| T1048.002 | SSH/SFTP transfers | stock rule 92603 (L6, file copied in over SCP — inbound only, auditd); outbound needs custom rules |
| T1560.001 | Archive staging | stock rules 92212 (L14), 92210 (L6), 91846 (L10) on Windows; custom rule 100530 (FIM on Linux staging dirs) |
| T1567.002 | Upload to cloud storage / webmail | stock rule 92038 (L12, script/binary connecting to a cloud resource); custom rules 100550, 100551 (proxy logs) |
| T1052 | USB mass storage | stock rule 81101 (L3, USB storage attached) + FIM on removable-media mount points |
| T1537 | Transfer to cloud account / exposed buckets | stock rules 99855 (L12), 99856 (L12) via AWS Security Hub; custom rules 100570, 100571, 100572 (CloudTrail / Azure / GCP audit) |
| DLP | DLP product alerts | custom rule 100560 on your DLP decoder |
...
> The XML below is an illustrative sketch of **org-custom** rules (IDs 100501+). Field names, groups (e.g. `dns`,
> `proxy`, `dlp`) and operators depend on your decoders — adapt and validate every rule with `wazuh-logtest` before
> deploying. None of these exist in the stock Wazuh ruleset.
...
## Approval Context Template (approvals queue rationale / team notification)

The response-planner agent puts this context in the `propose_action` rationale and case findings; approvers review it in the platform's approvals queue. The same text can be reused for a Slack/Teams notification to the on-call team.
