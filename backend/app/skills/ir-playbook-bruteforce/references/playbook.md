...
This playbook provides comprehensive procedures for detecting, analyzing, containing, eradicating, and recovering from brute force authentication attacks. Brute force attacks represent a persistent threat to organizational security, targeting authentication mechanisms across SSH, RDP, web applications, and other services. This playbook aligns with NIST SP 800-61r3 incident response lifecycle and integrates with the Wazuh Autopilot Strands agent swarm for automated detection and triage.
...
**Wazuh rules**: rules 5710 (L5), 5716 (L5), 5760 (L5), 5712 (L10), 5720 (L10), 5763 (L10); Windows rules 60122 (L5), 60204 (L10)
...
**Wazuh rules**: offline cracking leaves no authentication events. Watch for credential-store access instead: rules 92024 (L14, SAM hive copied from VSS), 92026 (L14, reg.exe SAM dump), 92900 (L12, LSASS read access) on Windows (Sysmon), and FIM on `/etc/shadow` (rule 550, L7) on Linux
...
**Wazuh rules**: no stock spraying rule. Lockouts across many accounts (rule 60115, L9, Event ID 4740) and cross-decoder frequency rule 40111 (L10) are the closest; add `custom rule 100210` (see Custom Rule Requirements)
...
**Wazuh rules**: no stock stuffing rule; add `custom rule 100211` (one account, many sources). A later success from a failing source raises rule 40112 (L12)

## Detection Criteria

### Wazuh Rule Coverage

All rule IDs and levels below are from the stock Wazuh 4.14 ruleset.

#### SSH Authentication Rules (0095-sshd_rules.xml)
| Wazuh rule | Level | Official description | Notes |
|---------|----------|-------------|----------------|
| rule 5710 | 5 | sshd: Attempt to login using a non-existent user | group `authentication_failed`, `invalid_login` |
| rule 5716 | 5 | sshd: authentication failed | single failure |
| rule 5760 | 5 | sshd: authentication failed | single failure (alternate log format) |
| rule 5758 | 8 | Maximum authentication attempts exceeded | per-connection limit hit |
| rule 5712 | 10 | sshd: brute force trying to get access to the system. Non existent user | frequency rule on rule 5710 |
| rule 5720 | 10 | sshd: Multiple authentication failures | frequency rule on rule 5716 |
| rule 5763 | 10 | sshd: brute force trying to get access to the system. Authentication failed | frequency rule on rule 5760 |
| rule 5715 | 3 | sshd: authentication success | correlate with failing sources |

#### PAM / syslog Authentication Rules
| Wazuh rule | Level | Official description | Notes |
|---------|----------|-------------|----------------|
| rule 5503 | 5 | PAM: User login failed | single failure |
| rule 5551 | 10 | PAM: Multiple failed logins in a small period of time | frequency rule on rule 5503 |
| rule 2501 | 5 | syslog: User authentication failure | generic syslog |
| rule 2502 | 10 | syslog: User missed the password more than one time | generic syslog |

#### Windows Authentication Rules (eventchannel, 0580-win-security_rules.xml)
| Wazuh rule | Level | Official description | Event ID |
|---------|----------|-------------|----------|
| rule 60122 | 5 | Logon Failure - Unknown user or bad password | Event ID 4625 |
| rule 60204 | 10 | Multiple Windows Logon Failures | Event ID 4625 (frequency) |
| rule 60115 | 9 | User account locked out (multiple login errors) | Event ID 4740 |
| rule 60106 | 3 | Windows Logon Success | Event ID 4624 |

Kerberos pre-authentication failures (Event ID 4771) and NTLM credential validation failures (Event ID 4776)
have no dedicated stock rules; add custom rules if domain controllers forward them.

#### Cross-source and Web Rules
| Wazuh rule | Level | Official description | Notes |
|---------|----------|-------------|----------------|
| rule 40111 | 10 | Multiple authentication failures | any decoder, same source IP, 12 events / 160 s |
| rule 40112 | 12 | Multiple authentication failures followed by a success | **escalate to critical** |
| rule 31510 | 8 | CMS (WordPress or Joomla) brute force attempt | web access logs |
| rule 31151 | 10 | Multiple web server 400 error codes | scanning or login hammering |

### Custom Rule Requirements

For comprehensive detection, organizations should implement custom rules (range 100000–120000, validated
with `wazuh-logtest`) for:

- **Password spraying** — `custom rule 100210`: frequency rule over group `authentication_failed`,
  `<same_source_ip />`, `<different_field>dstuser</different_field>`, ≥ 10 accounts in 30 min.
- **Credential stuffing** — `custom rule 100211`: `<same_field>dstuser</same_field>` with `<different_srcip />`.
...
historical_queries:
  ...
  - name: targeted_accounts
    query: "rule.id:(5710 OR 5712 OR 5716 OR 5760) AND data.dstuser:${username}"
    timerange: 4h
...
severity_scoring:
  ...
  modifiers:
    ...
    - condition: "account_lockout_triggered"
      adjustment: +1
      rule_id: [60115]  # Event ID 4740 lockout
...
correlation_rules:
  ...
  - name: account_lockout_pattern
    pattern:
      - rule.id: [60115]  # Event ID 4740 lockout
        unique_accounts: ">5"
        timeframe: 15m
  ...
  - name: vpn_then_internal
    pattern:
      - rule.groups: authentication_success  # from your VPN decoder (e.g. cisco-asa, pix)
        result: success
      - rule.groups: authentication_failed
        source: internal
        timeframe: 5m
...
queries:
  - name: unique_source_ips
    query: "rule.id:(5710 OR 5712 OR 5716 OR 5720 OR 5760 OR 5763) AND timestamp:[NOW-4h TO NOW]"
...
compromise_checks:
  ...
  - name: privilege_escalation
    query: "agent.name:${target_host} AND rule.mitre.tactic:\"Privilege Escalation\""
    timeframe: 1h
    indicators:
      - sudo_usage
      - runas_execution
...
|----------|------------|-------------|--------------|
| Event ID 4625 | Security | Failed logon attempt | Primary brute force indicator |
| Event ID 4624 | Security | Successful logon | Compromise verification |
| Event ID 4740 | Security | Account lockout | Attack impact indicator |
| Event ID 4771 | Security | Kerberos pre-auth failed | Domain-level brute force |
| Event ID 4776 | Security | Credential validation failed | NTLM authentication failure |
| Event ID 4648 | Security | Logon with explicit credentials | Possible credential reuse |
| Event ID 4768 | Security | Kerberos TGT requested | Kerberos authentication activity |
| Event ID 4769 | Security | Kerberos service ticket requested | Service access attempts |
...
### NIST SP 800-61r3 Incident Response Lifecycle (PICERL phases)
...
| **3. Containment** | - Short-term containment<br>- System backup<br>- Long-term containment<br>- Evidence preservation | - **response-planner** agent: action proposals (`propose_action`)<br>- Autonomy policy + approvals queue: risk gate and human approval<br>- Platform executor: MCP action execution; **responder** agent: verification | - Attack contained<br>- Evidence preserved<br>- Business impact minimized<br>- Approval documented |
...
| **5. Recovery** | - System restoration<br>- Service resumption<br>- Monitoring enhancement<br>- Verification testing | - **responder** agent: post-action verification (`wazuh_check_*`)<br>- Enhanced monitoring rules<br>- Post-recovery validation | - Systems operational<br>- Services restored<br>- Monitoring enhanced<br>- No recurrence detected |
...
| **Detect and Report** | Incident detection mechanisms | group `authentication_failures` (rules 5712, 5720, 5763, 60204, 40111, 40112) | Alert logs, correlation rules |
...
| **Autonomy policy + approvals queue** (platform, not an agent) | Risk and authorization gate | On every `propose_action` | - Enforce autonomy level and per-action rules<br>- Refuse protected targets<br>- Queue for human approval (RBAC)<br>- Audit the decision | Approved / rejected / auto-executed action |
| **responder** | Post-action verification | After the platform executes an approved action | - Verify with `wazuh_check_*` tools<br>- Record verification finding<br>- Recommend rollback if needed | Verification finding |
...
agent_workflow:
  ...
  phase_5_approval:
    ...
    gate: autonomy_policy_and_approvals_queue   # platform, not an agent
  ...
  phase_6_execution:
    ...
    executor: platform  # runs the Wazuh MCP action, then the responder agent verifies
    sla: 5_minutes
