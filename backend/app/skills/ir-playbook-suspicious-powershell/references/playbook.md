...
Stock Wazuh 4.14 rules (levels official). PowerShell/Operational rules live in `0915-win-powershell_rules.xml`
(group `powershell`); Sysmon rules in `0800`–`0950` (groups `sysmon_eid1_detections`, `sysmon_eid3_detections`, …).

| Wazuh rule | Level | Official description | Use |
|------------|-------|-------------|-----------------|
| rule 92057 | 12 | Powershell.exe spawned a powershell process which executed a base64 encoded command | encoded command |
| rule 91809 | 10 | Powershell script may be using Base64 decoding method | encoded/obfuscated script |
| rule 92071 | 12 | A powershell process created by WMI executed a base64 encoded command | WMI + encoded command |
| rule 92047 | 12 | Office application started mshta.exe | phishing chain |
| rule 92048 | 15 | Office application started mshta.exe and executed scripting command | phishing chain |
| rule 92156 | 12 | Office application loaded vbeui.dll module | macro execution |
| rule 92214 | 15 | Suspicious file created by Microsoft Office process | Office payload drop |
| rule 92029 | 6 | Powershell executed script from suspicious location | staging |
| rule 92065 | 6 | Powershell.exe launched by binary in a suspicious location | unusual parent |
| rule 91810 | 10 | Powershell script may be executing suspicious code with CreateThread API | in-memory execution |
| rule 91837 | 4 | Powershell executed "Get-Content -Stream or Invoke-Expresion" | string-to-code execution |
| rule 92151 | 12 | Binary loaded PowerShell automation library (unmanaged PowerShell) | PowerShell without powershell.exe |
| rule 92203 | 6 | Executable file created by powershell | payload drop |
| rule 92204 | 9 | Powershell process created executable file in AppData temp folder | payload drop |
| rule 92073 | 6 | Powershell executing certutil to decode a file | download/decode |
| rule 92008 | 12 | Windows Defender real time monitoring was disabled by Powershell command | defence tampering |
| rule 92024 | 14 | Powershell used to copy SAM hive from VSS | credential access |
| rule 92900 | 12 | Lsass process was accessed with read permissions, possible credential dumping | credential access (Sysmon Event ID 10) |
| rule 91844 | 12 | Possible addition of new item to Windows startup registry | persistence |
| rule 92226 | 14 | Powershell process has copied an executable file to Windows start-up folder | persistence |
| rule 91823 | 14 | Powershell script used "Invoke-command" cmdlet to execute code on remote computer | lateral movement |
| rule 92102 | 6 | Suspicious DCOM/RPC activity from Powershell process | network (Sysmon Event ID 3) |
| rule 92107 | 4 | Script generated suspicious network activity over TCP protocol | C2 (Sysmon Event ID 3) |

Discovery rules 91815, 91816, 91817, 91819 (L4) and rule 91818 (L4, `New-Service`) are context, not execution
indicators. The stock ruleset has **no** rules for execution-policy bypass, `-WindowStyle Hidden`, generic download
cradles or AMSI-bypass strings — create `custom rule 100310` (Sysmon Event ID 1 command line) and `custom rule 100311`
(script-block content) if you need them. Sysmon DNS queries (Event ID 22) have no stock rules either.
...
│  rules 92057, 92029, 92065 / SIGMA: wazuh-ps-*       │
└───────────────────┬─────────────────────────────────┘
                    │
                    ▼
┌─────────────────────────────────────────────────────┐
...
### Microsoft Defender Antivirus Events (includes AMSI detections)

| Windows event | Wazuh rule (level) | Description | Priority |
|----------|-------------------|-------------|----------|
| Event ID 1116 | rule 62123 (L12) | Defender detected malware or potentially unwanted software (AMSI-scanned scripts included) | Critical |
| Event ID 1117 | rule 62124 (L3) | Defender took action to protect the system | High |
| Event ID 1118 | rule 62125 (L8) | Defender failed to take remediation action | Critical |
| Event ID 1119 | rule 62126 (L14) | Defender hit a critical error during remediation | High |
...
### NIST SP 800-61r3 Incident Response Lifecycle Mapping
  ...
  response_planner:
    ...
    tasks:
      ...
      - task_id: generate_approval_request
        description: "Build the approval context for the approvals queue (propose_action rationale + case finding)"
      ...
      - task_id: await_approval_or_auto_execute
        description: "Autonomy policy decides: auto-execute if eligible, otherwise queue for human approval"
        logic:
          if: autonomy_policy_allows_autonomous_execution
            then: execute_containment_plan
          else:
            queue: approvals_queue (platform UI; optional Slack notification)
      ...
      - task_id: execute_containment_actions
        description: "Platform executor runs approved actions via the Wazuh MCP server (wazuh_* action tools)"
        actions:
          - name: kill_process
...
```
┌─────────────────────────────────────────────────────────┐
│  Wazuh Alert: Suspicious PowerShell (rule 92057)        │
...
3. **Wazuh Configuration**
   - Stock PowerShell/Operational rules (group `powershell`) and Sysmon rules (groups `sysmon_eid*_detections`) must be enabled; add custom rules 100310, 100311 for bypass/hidden-window/AMSI patterns
