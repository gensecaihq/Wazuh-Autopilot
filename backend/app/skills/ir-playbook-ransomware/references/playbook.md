...
Stock Wazuh 4.14 rules (levels official) plus org-custom rules the SOC must create.

#### File Integrity Monitoring (FIM, 0015-ossec_rules.xml)
| Wazuh rule | Level | Official description | Action |
|---------|-------|-------------|--------|
| rule 554 | 5 | File added to the system | Monitor; volume matters |
| rule 550 | 7 | Integrity checksum changed | Alert on bulk changes |
| rule 553 | 7 | File deleted | Alert on bulk deletions (originals removed after encryption) |
| rule 594 | 5 | Registry Key Integrity Checksum Changed | Windows persistence keys |
| rule 750 | 5 | Registry Value Integrity Checksum Changed | Windows persistence values |
| custom rule 100401 | 13 (suggested) | Mass FIM changes: ≥ 100 events from group `syscheck` in 60 s on one agent | **Immediate alert** |
| custom rule 100402 | 13 (suggested) | Ransom-note name or known encrypted extension in `syscheck.path` | **Immediate alert** |
| custom rule 100405 | 10 (suggested) | Changes in many directories at once on one agent | Alert |

There are no stock rules for "file modified" beyond rule 550; numbers such as 551/552 are not part of the 4.x ruleset.

#### Sysmon Event Monitoring (Sysmon rules in 0800–0870)
| Source | Stock Wazuh rules (level) | Detection logic |
|----------|-------------|-----------------|
| Sysmon Event ID 1 (process creation) | none for `vssadmin delete shadows`, `wmic shadowcopy delete`, `bcdedit ... recoveryenabled no`, `wbadmin delete catalog` → add `custom rule 100403` (level 15 suggested) | recovery inhibition |
| Sysmon Event ID 1 | rules 92008 (L12), 92009 (L13), 92012 (L13) — Defender protections disabled | defence tampering |
| Sysmon Event ID 1 | rules 92024 (L14), 92026 (L14) — SAM hive copied/dumped | credential theft before encryption |
| Sysmon Event ID 11 (file create) | rules 92213 (L15, executable in malware-typical folder), 92207 (L12, executable in Users\Public), 92214 (L15, Office-created suspicious file) | payload drop |
| Sysmon Event ID 3 (network) | rule 92104 (L15, suspicious binary opened a network connection) | C2 / exfil |
| Sysmon Event ID 10 (process access) | rule 92900 (L12, LSASS read access) | credential dumping |
| Sysmon Event ID 23 (file delete) | no stock rules | add custom rules if collected |

#### Antimalware, Rootcheck and Log Tampering
| Wazuh rule | Level | Official description |
|---------|-------|-----------|
| rule 62113 | 14 | Windows Defender: Antimalware engine found malware or other potentially unwanted software |
| rule 62122 | 12 | Windows Defender: Antimalware platform detected suspicious behaviour |
| rule 87105 | 12 | VirusTotal: file detected by one or more engines (VirusTotal integration) |
| rule 510 | 7 | Host-based anomaly detection event (rootcheck) |
| rule 521 | 11 | Possible kernel level rootkit |
| rule 60117 | 9 | Windows audit log was cleared (Event ID 1102) |
| rule 99594 | 15 | MS Graph: system infected with ransomware (Microsoft Graph integration) |
| custom rule 100404 | 12 (suggested) | VSS or backup service stopped (System log Event IDs 7036 / 7040) |
...
correlation_rules:
  primary_alerts:
    - rule.groups: "syscheck" AND rule.level: >=7
    - rule.groups: "sysmon" AND event.id: [1, 11, 23]
    - rule.groups: "windows_security" AND rule.id: [60117]  # audit log cleared
    - rule.groups: "windows_defender" AND rule.level: >=12
...
|--------|----------|-------------|-----------|
| **FIM Alerts** | CRITICAL | `rule.groups:syscheck AND agent.id:${id} AND rule.level:>=7` | 90 days |
...
### Multi-Agent Coordination (Strands swarm)

```yaml
agent_roles:   # roster ids; handoffs use handoff_to_agent
  triage:
    responsibility: "Validate Wazuh alerts, open/attach the case, extract entities"
    triggers: "rule.level:>=12 AND rule.groups:ransomware"  # group set by the custom rules 100401-100405
    handoff_to: "correlation"

  correlation:
    responsibility: "Fleet-wide correlation, ATT&CK mapping, timeline"
    sla: "Complete within 5 minutes"
    handoff_to: "investigation"

  investigation:
    responsibility: "Host forensics, blast radius, evidence collection"
    sla: "Findings within 30 minutes"
    handoff_to: "response-planner"

  response-planner:
    responsibility: "Containment proposals via propose_action (autonomy policy + approvals queue decide execution)"
    sla: "Proposals within 2 minutes"
    handoff_to: "reporting"

  reporting:
    responsibility: "Stakeholder/executive incident report (save_report)"
    triggers: "Major escalation events"
    handoff_to: "incident_commander (human)"
```
