...
Stock Wazuh 4.14 rules (levels official).

| Wazuh rule | Level | Official description | Platform | Use |
|---------|----------|-------------|----------|-----------------|
| rule 5401 | 5 | Failed attempt to run sudo | Linux | escalation attempt |
| rule 5404 | 10 | Three failed attempts to run sudo | Linux | repeated attempts |
| rule 5405 | 5 | Unauthorized user attempted to use sudo | Linux | user not in sudoers |
| rule 5402 | 3 | Successful sudo to ROOT executed | Linux | baseline; alert on unusual user/command |
| rule 5403 | 4 | First time user executed sudo | Linux | new sudo user |
| rule 5302 | 9 | User missed the password to change UID to root | Linux | failed su to root |
| rule 5303 | 3 | User successfully changed UID to root | Linux | su to root |
| rule 80721 | 10 | Auditd: User becomes root | Linux (auditd) | root transition |
| rule 80742 | 5 | Auditd: User ID changed | Linux (auditd) | UID change |
| rule 5902 | 8 | New user added to the system | Linux | new (possibly UID 0) account |
| rule 2961 | 5 | User added to group sudo | Linux | sudo group change |
| rule 2833 | 8 | Root's crontab entry changed | Linux | cron persistence |
| rule 5132 | 11 | Unsigned kernel module was loaded | Linux | kernel exploit / rootkit |
| rule 550 | 7 | Integrity checksum changed | Linux/Windows (FIM) | `/etc/sudoers*`, `/etc/passwd`, `/etc/shadow`, systemd units — paths must be monitored |

Note: the PAM session rules (rule 5501 session opened, rule 5502 session closed, rule 5503 login failed) and rule 5551
(multiple failed PAM logins) describe logins, not privilege elevation. The auditd watch rules 80790/80791 are generic
file created/deleted events and only indicate escalation when the watched path is sensitive.

### Extended Detection Rules

#### Linux Privilege Escalation Rules (org-custom — not in the stock ruleset)

Create these in the custom range and validate with `wazuh-logtest`:

| Custom rule | Description | Detection Pattern |
|---------|-------------|-------------------|
| custom rule 100610 | SUID binary execution with unusual arguments | auditd execve on SUID binaries with GTFOBins-style arguments |
| custom rule 100611 | Kernel exploit signature detected | known exploit binaries/patterns in auditd execve |
| custom rule 100612 | Suspicious capability modification | auditd execve of `setcap` |
| custom rule 100613 | Polkit privilege escalation attempt | `pkexec` abuse |
| custom rule 100614 | Sudoers file modification | child of FIM rule 550 on `/etc/sudoers` or `/etc/sudoers.d/*`, raised to level 12 |
| custom rule 100615 | Cron job created by non-root user | FIM / cron logs |
| custom rule 100616 | Systemd service created outside package manager | FIM rule 554 under `/etc/systemd/system` |
| custom rule 100617 | LD_PRELOAD or LD_LIBRARY_PATH manipulation | auditd / FIM on `/etc/ld.so.preload` |
| custom rule 100618 | /etc/passwd or /etc/shadow access by web process | auditd read watch by www-data/apache |

#### Windows Privilege Escalation Rules

| Source | Wazuh rule (level) | Description |
|----------|---------|-------------|
| Event ID 4672 | rule 67028 (L3) | Special privileges assigned to new logon — meaningful for non-admin accounts |
| Event ID 4732 | rules 60144 (L5), 60154 (L12, Administrators group changed), 60179 (L10, Remote Desktop Users group changed) | local/admin group membership change |
| Event ID 7045 | rule 61138 (L5) | New Windows service created (System log) |
| Event ID 4698 | rule 60228 (L4) | Scheduled task created |
| Sysmon Event ID 1 | rules 92046 (L12, fodhelper UAC bypass), 92055 (L12), 92056 (L14) | auto-elevated utility abuse |
| Sysmon Event ID 13 | rules 92304 (L6), 92305 (L12), 92306 (L12) | UAC-bypass registry keys |
| Sysmon Event IDs 8 / 10 | rules 92400 (L12), 92910 (L12), 92900 (L12) | process injection / LSASS access |
| — | custom rule 100651 | Token manipulation via Windows API (OpenProcessToken, DuplicateTokenEx) — no stock rule |
| — | custom rule 100652 | SeDebugPrivilege enabled for a non-system process (Event ID 4703) — no stock rule |

Event IDs 4673, 4674 and 4688 have no stock Wazuh rules; collect Sysmon for process-creation coverage.
...
baseline_queries:
  user_sudo_history:
    query: "srcuser:${actor_user} AND rule.id:(5402 OR 5403 OR 5303)"
  ...
  host_privilege_patterns:
    query: "agent.id:${agent_id} AND rule.mitre.tactic:\"Privilege Escalation\""
...
**Example Calculation:**
```
Alert: rule 5402 (sudo to root) = Base 6 (Medium)
...
correlation_engine:
  ...
  cluster_rules:
    ...
    - name: "Post-Exploitation Activity"
      ...
      sequence:
        - step1: rule.mitre.tactic:"Privilege Escalation"
        - step2: rule.mitre.tactic:("Credential Access" OR "Lateral Movement")
    ...
    - name: "Reconnaissance to Escalation"
      ...
      sequence:
        - step1: rule.mitre.tactic:"Discovery"
        - step2: rule.mitre.tactic:"Privilege Escalation"
...
timeline_builder:
  ...
  event_sources:
    authentication:
      query: "srcuser:${actor_user} AND rule.groups:(authentication_success OR authentication_failed)"
      importance: high
    ...
    network_connections:
      query: "agent.id:${agent_id} AND rule.groups:firewall"
      importance: high
...
threat_hunting_queries:
  hunt_for_similar_activity:
    - name: "Same user on other hosts"
      query: "srcuser:${actor_user} AND rule.mitre.tactic:\"Privilege Escalation\""
      timeframe: "last 30 days"
    ...
    - name: "Same source IP attempting escalation"
      query: "data.src_ip:${source_ip} AND rule.mitre.tactic:\"Privilege Escalation\""
      timeframe: "last 14 days"

  hunt_for_persistence:
    - cron_jobs_created: "rule.id:2833 OR rule.id:100615"  # stock root-crontab rule + custom rule
    - services_installed: "rule.id:(61138 OR 100616)"  # stock Windows service rule + custom rule
    - scheduled_tasks: "rule.id:60228"
    - suid_modifications: "rule.id:100612"  # custom rule
  ...
  hunt_for_lateral_movement:
    ...
    - ssh_sessions: "rule.groups:sshd AND rule.id:5715"
...
| AC-6 | Least Privilege | Privilege escalation detection, unauthorized elevation alerting | rules 5402, 5303, 80721, privilege audit logs |
...
|-------------|-------------|----------------|----------|
| 10.2.2 | Log all actions taken by any individual with root or administrative privileges | All sudo/su/admin actions logged | rules 5402, 5303, 80721, 67028 (Event ID 4672) |
| 10.2.5 | Log unauthorized access attempts to data | Detects unauthorized privilege escalation | rules 5401, 5404, 5302, failed elevation attempts |
...
agent_pipeline:
  ...
  stage_4_policy_validation:
    gate: autonomy_policy_and_approvals_queue  # platform policy engine, not an agent
    trigger: containment_action_proposed  # response-planner called propose_action
    responsibilities:
  ...
  stage_5_response_execution:
    executor: platform  # runs the Wazuh MCP action; responder agent verifies
...
inter_agent_messaging:
  ...
  example_policy_decision:
    from: autonomy_policy  # platform decision recorded in the audit log
    to: approvals_queue
    message:
...
soc_dashboard:
  ...
  agent_performance:
    ...
    - action_verification_success_rate
...
```json
{
  ...
  "techniques": [
    ...
    {"techniqueID": "T1548.003", "score": 100, "comment": "Sudo logging, rule 5402, timestamp monitoring"},
