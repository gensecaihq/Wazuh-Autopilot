...
#### T1021.004 - SSH
- **Detection**: Wazuh rule 5715 (L3, sshd authentication success) from internal sources, preceded by rules 5716, 5760 (L5, failures); rule 92603 (L6, file copied in via SCP, auditd)
...
All rule IDs and levels are from the stock Wazuh 4.14 ruleset. Most Windows detections need Sysmon and the
Security/PowerShell eventchannels forwarded to Wazuh.

#### SSH-Based Lateral Movement (0095-sshd_rules.xml, 0850-audit_rules.xml)
| Wazuh rule | Level | Official description | Use |
|---------|-------|-------------|----------|
| rule 5715 | 3 | sshd: authentication success | baseline; internal source to many targets = movement |
| rule 5716 | 5 | sshd: authentication failed | failed hop attempts |
| rule 5760 | 5 | sshd: authentication failed | failed hop attempts (alternate format) |
| rule 5720 | 10 | sshd: Multiple authentication failures | spraying internal hosts |
| rule 92603 | 6 | A file was copied to this system over SSH using SCP | tool transfer |
| rule 92602 | 12 | Suspicious python script matches Impacket signature | Impacket from a Linux host |

**Correlation logic**: rule 5715 events for one account from a single source to ≥ 3 targets within 10 minutes = lateral
movement pattern. There is no stock rule for this fan-out; create `custom rule 100230`
(frequency over group `authentication_success`, `<same_field>dstuser</same_field>`, `<different_field>agent.id</different_field>`).

#### Windows Remote Execution
| Wazuh rule | Level | Official description | Technique |
|---------|-------|-------------|----------|
| rule 92068 | 3 | PSEXEC was used to execute a command | T1569.002 (Sysmon Event ID 1) |
| rule 61138 | 5 | New Windows Service Created | T1543.003 (System Event ID 7045) |
| rule 92650 | 12 | New Windows Service Created to start from windows root path (binary likely dropped via admin shares) | T1021.002 / T1569.002 |
| rule 92202 | 6 | Executable dropped in Windows root folder | T1570 (Sysmon Event ID 11) |
| rule 92218 | 6 | Possible abuse of Windows admin shares by binary dropped in Windows root folder by system process | T1570 |
| rule 92105 | 3 | Possible suspicious access to Windows admin shares | T1021.002 (Sysmon Event ID 3) |
| rule 92106 | 3 | Windows System process activity over SMB port | T1021.002 |
| rule 92070 | 6 | Windows management instrumentation (WMI) created a powershell process | T1047 |
| rule 92071 | 12 | A powershell process created by WMI executed a base64 encoded command | T1047 / T1059.001 |
| rule 92110 | 4 | Detected WinRM activity | T1021.006 (Sysmon Event ID 3) |
| rule 91823 | 14 | Powershell script used "Invoke-command" cmdlet to execute code on remote computer | T1021.006 |
| rule 92220 | 6 | Binary created in Windows root folder by WinRM process | T1570 |
| rule 60228 | 4 | A scheduled task was created | T1053.005 (Event ID 4698) |

#### Windows Authentication Events (eventchannel)
| Wazuh rule | Level | Official description | Technique |
|---------|-------|-------------|----------|
| rule 60106 | 3 | Windows Logon Success | baseline (Event ID 4624) |
| rule 92652 | 6 | Successful Remote Logon Detected — NTLM authentication, possible pass-the-hash | T1550.002 |
| rule 92657 | 6 | Successful Remote Logon Detected — NTLM authentication over RDP, possible pass-the-hash | T1550.002 / T1021.001 |
| rule 92653 | 3 | User logged using Remote Desktop Connection (RDP) | T1021.001 |
| rule 92656 | 15 | User logged using RDP from loopback address, possible tunnelling | T1021.001 |
| rule 60204 | 10 | Multiple Windows Logon Failures | failed spread attempts (Event ID 4625) |
| rule 67028 | 3 | Special privileges assigned to new logon | Event ID 4672 |
| rule 92900 | 12 | Lsass process was accessed with read permissions, possible credential dumping | T1003.001 (Sysmon Event ID 10) |

Explicit-credential logons (Event ID 4648), share access (Event IDs 5140/5145) and Kerberos ticket requests
(Event IDs 4768/4769) have no dedicated stock rules; add custom rules (e.g. `custom rule 100231`) if you depend on them.
...
|----------|-------------|---------------------------|
| **Event ID 4624** | Successful logon | Type 3 (Network), Type 10 (RemoteInteractive) from internal IPs |
| **Event ID 4648** | Logon with explicit credentials | Indicates credential usage for remote operations |
| **Event ID 4672** | Special privileges assigned | Admin rights used on remote system |
| **Event ID 5140** | Network share accessed | ADMIN$, C$, IPC$ access patterns |
| **Event ID 5145** | Detailed file share access | File operations on remote shares |
| **Event ID 7045** | New service installed | Remote service creation (PsExec pattern) |
| **Event ID 1102** | Audit log cleared | Anti-forensics after lateral movement |
| **Event ID 4776** | NTLM authentication | Pass-the-Hash indicator when without prior Kerberos |
| **Event ID 4768** | Kerberos TGT requested | Baseline for Pass-the-Ticket detection |
| **Event ID 4769** | Kerberos Service Ticket requested | Unusual ticket requests, encryption downgrade |
...
#### Host Isolation
```bash
# Human/break-glass only. In Autopilot, isolation is proposed by response-planner and executed by the
# platform after approval; agents never call the Wazuh API directly.
...
- **SSH**: rules 5715, 5716, 5760, 5720; rules 92603, 92602 (auditd)
- **Windows remote execution**: rules 92068 (PsExec), 61138, 92650 (services), 92070, 92071 (WMI), 92110, 91823 (WinRM), 60228 (scheduled task)
- **Windows authentication**: rules 92652, 92657 (NTLM / pass-the-hash), 92653, 92656 (RDP), 60204 (logon failures), 92900 (LSASS access)

### Critical Windows Events
- **Event ID 4624 Type 3**: Network logon (SMB, WMI)
- **Event ID 4624 Type 10**: Remote interactive (RDP)
- **Event ID 4648**: Explicit credentials (remote execution)
- **Event IDs 5140/5145**: Share access (ADMIN$, C$)
- **Event ID 7045**: New service (PsExec indicator)
